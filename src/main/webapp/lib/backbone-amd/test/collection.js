(function() {
  ...
  test("can't add models to collection twice", function() {
    var col = new Backbone.Collection([{id: 1}, {id: 2}, {id: 1}, {id: 2}, {id: 3}]);
    equal(col.pluck('id').join(' '), '1 2 3');
  });

  test("can't add different models with same id to collection twice", 1, function() {
    var col = new Backbone.Collection;
    col.unshift({id: 101});
    col.add({id: 101});
    equal(col.length, 1);
  ...
  test("add models to multiple collections", 10, function() {
    var counter = 0;
  ...
  test("add models with parse", 1, function() {
  ...
  test("add models to collection with sort()-style comparator", 3, function() {
    var col = new Backbone.Collection;
    col.comparator = function(a, b) {
  ...
  test("remove same models in multiple collection", 16, function() {
    var counter = 0;
  ...
  test("models destroy removes from all collections", 3, function() {
    var e = new Backbone.Model({id: 5, title: 'Othello'});
  ...
  test("Colllection: non-persisted models destroy removes from all collections", 3, function() {
  ...
  test("a failing create returns models with errors", function() {
    var ValidatingModel = Backbone.Model.extend({
      validate: function(attrs) {
        return "fail";
      }
  ...
  test("reset passes caller options", 3, function() {
    ...
    col.reset([{ astring: "green", anumber: 1 }, { astring: "blue", anumber: 2 }], { model_parameter: 'models parameter' });
    equal(col.length, 2);
    col.each(function(model) {
      equal(model.model_parameter, 'models parameter');
    });
  });
  ...
  test("#714: access `models.collection` in a brand new models.", 2, function() {
    var collection = new Backbone.Collection;
    collection.url = '/test';
    var Model = Backbone.Model.extend({
      set: function(attrs) {
  ...
  test("multiple copies of the same models", 3, function() {
    var col = new Backbone.Collection();
  ...
  test("#1112 - passing options.models sets collection.model", 2, function() {
    var Model = Backbone.Model.extend({});
  ...
  test("#1447 - create with wait adds models.", 1, function() {
    var collection = new Backbone.Collection;
    var model = new Backbone.Model;
  ...
  test("`set` and models level `parse`", function() {
    var Model = Backbone.Model.extend({});
    var Collection = Backbone.Collection.extend({
      model: Model,
  ...
  test("`add` only `sort`s when necessary", 2, function () {
    ...
    collection.add({id: 4}); // do sort, new models
  ...
  test("`add` only `sort`s when necessary with comparator function", 3, function () {
    ...
    collection.on('sort', function () { ok(true); });
    collection.add({id: 4}); // do sort, new models
    collection.add({id: 1, a: 1}, {merge: true}); // do sort, models change
    collection.add({id: 1, b: 1}, {merge: true}); // do sort, models change
    collection.add({id: 1, a: 1}, {merge: true}); // don't sort, no models change
    collection.add(collection.models); // don't sort, nothing new
    collection.add(collection.models, {merge: true}); // don't sort
  });
