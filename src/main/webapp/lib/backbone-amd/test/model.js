(function() {
  ...
  test("non-persisted destroy", 1, function() {
    ...
    a.destroy();
    ok(true, "non-persisted models should not call sync");
  });
  ...
  test("#1433 - Save: An invalid models cannot be persisted.", 1, function() {
    var model = new Backbone.Model;
  ...
  test("#1545 - `undefined` can be passed to a models constructor without coersion", function() {
    var Model = Backbone.Model.extend({
      defaults: { one: 1 },
      initialize : function(attrs, opts) {
  ...
  test("#1961 - Creating a models with {validate:true} will call validate and use the error callback", function () {
    var Model = Backbone.Model.extend({
